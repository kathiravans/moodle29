...
class auth_plugin_db extends auth_plugin_base {
    ...
    function user_login($username, $password) {
        ...
        if ($this->is_internal()) {
            ...
            if ($this->config->passtype === 'plaintext') {
            ...
            } else if ($this->config->passtype === 'aes') {          
                //$passwordkey = $authdb->Execute("SELECT * FROM iplat_configurations WHERE iplat_configurations.key = 'ENCRYPTION_KEY'");
                //$fetch_obj = $passwordkey->FetchObj();
                $rs = $authdb->Execute("SELECT * FROM {$this->config->table}
                WHERE {$this->config->fielduser} = '".$this->ext_addslashes($extusername)."'
                AND {$this->config->fieldpass} = AES_ENCRYPT('".$this->ext_addslashes($extpassword)."','ieyptrueitn')");
                $fields = $rs->FetchRow();
                $authdb->Close();
                $rs->Close();
                if($fields) {
                ...
                //return (strtolower($fromdb) == $extpassword);    
