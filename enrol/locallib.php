...
class course_enrolment_manager {
    ...
    protected function execute_search_queries($search, $fields, $countfields, $sql, array $params, $page, $perpage, $addedenrollment=0) {
        ...
        foreach($availableusers as $key=>$value) { // hangouts kathir
      
            $inscheck = $DB->get_records_sql("SELECT * from mdl_role_assignments
            JOIN mdl_context ON
            mdl_role_assignments.contextid=mdl_context.id
            JOIN mdl_course ON
            mdl_context.instanceid=mdl_course.category
            JOIN mdl_enrol ON
            mdl_enrol.courseid=mdl_course.id
            and mdl_enrol.id=$params[enrolid] and mdl_role_assignments.userid=$key");
            if(!$inscheck){
    ...
    public function get_potential_users($enrolid, $search='', $searchanywhere=false, $page=0, $perpage=25, $addedenrollment=0) {
        ...
        //$enroltable = $DB->get_record('enrol', array('id' =>$enrolid));
        //
        //$course = $DB->get_record('course', array('id' =>$enroltable->courseid));
        //
        //$enroltable = $DB->get_record('course', array('id' =>$course->courseid));
        //
        //
        ...
        //JOIN mdl_enrol ON
        //mdl_enrol.courseid=mdl_course.id
        //and mdl_enrol.id=$enrolid and mdl_role_assignments.userid=41
