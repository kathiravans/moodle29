...
class core_user_external extends external_api {
   ...
   public static function user_login_auth_parameters() {
        global $CFG;
     ...
     public static function user_login_auth($users) { // kathir
        ...
        $params = self::validate_parameters(self::user_login_auth_parameters(), array('users' => $users));
        ...
        foreach ($params['users'] as $user) {
            ...
            if ($DB->record_exists('user', array('username' => $user['username']))) {
                ...
                $validuser = authenticate_user_login($frm->username, $frm->password);
                $userids[] = array('id' => $validuser->id, 'username' => $validuser->username);
            } else {
                throw new invalid_parameter_exception('Username not exists: '.$user['username']);
            }
        ...
        //$login = complete_user_loginss($user,$sessionid);
        
        if ($DB->record_exists('user', array('username' => $username))) {
            $userrec = $DB->get_record('user', array('username' => $username));
            $secretkey = bin2hex(mcrypt_create_iv(7, MCRYPT_DEV_RANDOM));
            $record = new \stdClass();
            $record->id=$userrec->id;
            $record->auth = 'email';
            $record->confirmed = '0';
            ...
            $userids[] = array('secretid' => $secretkey, 'username' => $userrec->username);
        } else {
            throw new invalid_parameter_exception('Username not exists for updating secret token: '.$username);
        }
        $transaction->allow_commit();
        return $userids;
    }
    
     public static function user_login_auth_returns() {
        return new external_multiple_structure(
            new external_single_structure(
                array(
                    'secretid'       => new external_value(PARAM_RAW, 'secret key'),
                    'username' => new external_value(PARAM_USERNAME, 'user name'),
                )
            )
        );
